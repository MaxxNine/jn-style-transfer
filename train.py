...
class Trainer:
    def __init__(self,
        ...
        self.transform = self.load_model_if_exists()
    ...
    def load_model_if_exists(self):
        model = TransformNet()  # Always start with a TransformNet instance
        weights_path = os.path.join(self.saved_model_path, f"{self.style_name}.h5")
        if os.path.exists(weights_path):
            try:
                model.model.load_weights(weights_path)  # Load weights into the model attribute of TransformNet
                print(f"Model weights loaded from {weights_path}")
            except Exception as e:
                print(f"Couldn't load weights. Details: {str(e)}")
        else:
    ...
    def _save_protocol(self):
        ...
        self.transform.model.save_weights(os.path.join(self.saved_model_path, f"{self.style_name}.h5"))
        print(f"Model weights saved to {self.saved_model_path}")
