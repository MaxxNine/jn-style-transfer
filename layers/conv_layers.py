from keras.layers import Activation, Add, Conv2D, Conv2DTranspose, Layer
...
class ConvLayer (Layer):
    def __init__(self, filters, 
        ...
        self.conv = Conv2D(filters, kernel_size=kernel, padding=padding, 
                           strides=strides, use_bias=False, name=name,
...
class ResBlock(Layer):
  def __init__(self, filters, kernel=(3,3), padding='same', weight_initializer="glorot_uniform", prefix=""):
    ...
    self.conv1 = ConvLayer(filters=filters, kernel=kernel, 
                           weight_initializer=weight_initializer, padding=padding, name=self.prefix_name + "conv_1")
    self.conv2 = ConvLayer(filters=filters, kernel=kernel, padding=padding, 
                           weight_initializer=weight_initializer, activate=False, name=self.prefix_name + "conv_2")
